...
namespace oceanbase
{
  namespace common
  {
    ...
    uint32_t murmurhash32(const void *data, int32_t len, uint32_t hash);
    ...
    struct MurmurHash32
    {
      ...
      {
        return murmurhash32(s.c_str(), static_cast<int32_t>(s.length()), 0);
      }

      uint32_t operator()(const void *start, int32_t len) const
      {
        return murmurhash32(start, len, 0);
      }

      uint32_t operator()(const void *start, int32_t len, uint32_t seed) const
      {
        return murmurhash32(start, len, seed);
      }

      uint32_t operator()(const char *s) const
      {
        return murmurhash32(s, static_cast<int32_t>(strlen(s)), 0);
      }
    };
