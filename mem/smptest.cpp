...
//	std::cout  << "L1 cache hit ratio:"
//			<< getL1CacheHit(before_sstate, after_sstate) << std::endl
//			<< "L2 cache hit ratio:"
//			<< getL2CacheHit(before_sstate, after_sstate) << std::endl
//			<< "L3 cache hit ratio:"
//			<< getL3CacheHit(before_sstate, after_sstate) << std::endl;
	dump(before_sstate, after_sstate);
}

/**
 * memory access thread
 */
void *smpmemthread(void *arg) {
...
//	CoreCounterState after_sstate;
	CoreState before_sstate;
	CoreState after_sstate;
...
////		return NULL;
//	}
	if (initpcm() != PCM::Success) {
		VOLT_WARN("PCM does not open successfully");
	}

//	printf("cpu model: %d", m->getCPUModel());
...
//		before_sstate = getCoreCounterState(0);
		getCoreCounterState(before_sstate);
...
//		after_sstate = getCoreCounterState(0);
		getCoreCounterState(after_sstate);
